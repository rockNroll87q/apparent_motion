...
filename = _thisDir + os.sep + u'data_mapping/%s_%s_%s_mapping' % (expInfo['participant'], expName, expInfo['date'])
...
Cycles = data.TrialHandler(nReps=1.0, method='sequential',
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('ExpConditions_mapping.xlsx'),
...
for thisCycle in Cycles:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    print(str(Condition) + '\t' + str(eventClock.getTime()))
    respFile.write(str(Condition) + '\t' + str(eventClock.getTime()) + '\n')
